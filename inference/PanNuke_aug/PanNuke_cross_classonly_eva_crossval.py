...
def run_nuclei_type_stat(pred_dir, true_dir, txt_fp, type_uid_list=[1, 2, 3, 5], exhaustive=True):
    ...
    """
    idx = 6
    txt_fp.writelines('Epithelial tp_dt: ' + str(results_list[idx][0]) + '\n')
    txt_fp.writelines('Epithelial tn_dt: ' + str(results_list[idx][1]) + '\n')
    txt_fp.writelines('Epithelial fp_dt: ' + str(results_list[idx][2]) + '\n')
    txt_fp.writelines('Epithelial fn_dt: ' + str(results_list[idx][3]) + '\n')
    txt_fp.writelines('Epithelial fp_d: ' + str(results_list[idx][4]) + '\n')
    txt_fp.writelines('Epithelial fn_d: ' + str(results_list[idx][5]) + '\n')
    """
...
if __name__ == "__main__":
    ...
    for cat in ['1', '2', '3', '5', 'binary']:
        test_gt_root = '/home/jianan/InstanceSeg+Class/raw_datasets/PanNuke/raw/test' + train_val + '/' + target_organ + '_mask_patches/' + cat + '/'
        txt_path = '../../InferenceResult/mrcn_predict_inpaint_nuc_cor_class/PanNuke_' + target_organ + '/fold' + train_val[-1] + '/' + source_organ + tail_name + '/eva_strict_' + cat + '.txt'
        fp = open(txt_path, "w")
        for i in range(1, 2):
            if i < 10:
                model_index = '00%d' % i
                ...
                model_index = '0%d' % i
            test_pred_root = '../../InferenceResult/mrcn_predict_inpaint_nuc_cor_class/PanNuke_' + target_organ + '/fold' + train_val[-1] + '/' + source_organ + tail_name + '/' + model_index + '/ins_mask/' + cat + '/'
            evaluate_fluo2tcga(test_pred_root, test_gt_root, fp, min_size)
        fp.close()
    
    
    fp = open('../../InferenceResult/mrcn_predict_inpaint_nuc_cor_class/PanNuke_' + target_organ + '/fold' + train_val[-1] + '/' + source_organ + tail_name + '/eva_strict_avg.txt', 'w')
    cat_pq = np.zeros([4, 1])
    for idx, cat in enumerate(['1', '2', '3', '5']):
        cat_txt = open('../../InferenceResult/mrcn_predict_inpaint_nuc_cor_class/PanNuke_' + target_organ + '/fold' + train_val[-1] + '/' + source_organ + tail_name + '/eva_strict_' + cat + '.txt', 'r')
    ...
    '''
    fp = open('../../InferenceResult/mrcn_predict_inpaint_nuc_cor_class/PanNuke_' + target_organ + '/fold' + train_val[-1] + '/' + source_organ + tail_name + '/eva_class_nodet.txt', "w")
    true_dir = '/home/jianan/InstanceSeg+Class/raw_datasets/PanNuke/raw/test' + train_val + '/' + target_organ + '_mask_patches/class/'
    for i in range(1, 11):
        if i < 10:
            model_index = '00%d' % i
        else:
            model_index = '0%d' % i
        pred_dir = '../../InferenceResult/mrcn_predict_inpaint_nuc_cor_class/PanNuke_' + target_organ + '/fold' + train_val[-1] + '/' + source_organ + tail_name + '/' + model_index + '/class/'
