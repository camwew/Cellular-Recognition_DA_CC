...
class ImageLabelResizeLayer(nn.Module):
    ...
    def forward(self,x, need_backprop):
        ...
        gt_blob = np.zeros((feats.shape[0], feats.shape[2], feats.shape[3], 1), dtype=np.float32)
        for i in range(feats.shape[0]):
            lb=np.array([lbs[0]])
            lbs_resize = cv2.resize(lb, (feats.shape[2] ,feats.shape[3]),  interpolation=cv2.INTER_NEAREST)
